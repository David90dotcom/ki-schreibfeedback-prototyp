# KI-Schreibfeedback-Prototyp 0.10 (Experiment)

Web-App-Prototyp zur geschützten Erzeugung von Schreibfeedback mit OpenAI, der Mistral-Cloud-API, lokalem Ollama und einem selbst betriebenen Ministral-Modell über RunPod Serverless. Der Teststand 0.10 bietet drei vergleichbare Analyseverfahren: eine gemeinsame Anfrage für alle Kriterien, eine kriterienweise Einzelanalyse mit genau einem fokussierten Modellaufruf je Kriterium sowie eine experimentelle Zwei-Pass-Prüfung. Bei allen Providern ist die gemeinsame Analyse vorausgewählt; die aufwendigeren Varianten müssen bewusst aktiviert werden.
...
- im bisherigen Kontrollmodus genau eine Modellanfrage für alle Kriterien einer Feedback-Vorlage
- optionale kriterienweise Analyse mit genau einem sequenziellen, fokussierten Modellaufruf je Kriterium
- gezielte manuelle Aktualisierung einer einzelnen Ergebniskarte mit genau einem zusätzlichen Kriterienaufruf
- optionaler Zwei-Pass-Versuch mit zwei aufeinanderfolgenden Anfragen an dasselbe ausgewählte Modell
...
Unter „Feedback“ wird eine Aufgabe gemeinsam mit genau einer Feedback-Vorlage angelegt. Die Vorlage besteht aus mehreren einzeln sortierbaren Feedback-Kriterien. Im Analyseformular kann anschließend die gespeicherte Aufgabe ausgewählt werden. In der vorausgewählten gemeinsamen Analyse werden Aufgabe, Material, alle Kriterien und Schülertext gemeinsam in genau einer Anfrage an den gewählten Provider übermittelt. Alternativ stehen die kriterienweise Einzelanalyse und die Zwei-Pass-Prüfung zur Verfügung. Die bestehenden JSON-Vorlagen bleiben dabei unverändert kompatibel; weder ihr Austauschformat noch bereits importierte Aufgaben müssen für eines der drei Verfahren angepasst werden.
